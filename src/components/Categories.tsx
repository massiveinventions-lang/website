      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {categories.map((cat, idx) => {
            ...
            // Categories with an `href` become real navigation; the rest
            // stay as visual-only tiles (so the existing hover effects
            // and layout continue to work for them).
            if ("href" in cat && cat.href) {
              return (
                <Link key={cat.title} href={cat.href} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--retro-cream)] rounded-3xl">
                  {card}
                </Link>
              );
            }
